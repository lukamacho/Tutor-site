...
class InMemoryLessonRepository:
    ...
    def create_lesson(
        ...
        lesson = Lesson(subject, tutor_mail, student_mail, number_of_lessons, lesson_price)
        self.data.append(lesson)
        return lesson

    def get_lesson(self, tutor_mail: str, student_mail: str, subject: str) -> Lesson:
        for lesson in self.data:
            if lesson.subject == subject and lesson.tutor_mail == tutor_mail and lesson.student_mail == student_mail:
                return lesson
        return None
